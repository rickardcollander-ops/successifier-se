...
**1. Auktoritetssignaler.** Domänauktoritet, antalet inkommande länkar från trovärdiga sajter och omnämnanden i akademiska eller journalistiska källor. En oberoende recension på G2 eller ett omnämnande i en [Forrester Wave](https://www.forrester.com/research/forrester-wave/) väger tyngre än tio egenpublicerade blogginlägg.
...
Perplexity är det mest transparenta verktyget eftersom det visar sina citeringar. [analyser från SEO-byråer som NP Digital](https://www.npdigital.com/blog/) visar att Perplexity tenderar att citera:
...
- Plats i analysthus-rapporter ([Gartner, Forrester, IDC](https://www.gartner.com/en/research))
...
- `FAQPage`: markerar frågor och svar så att de lätt kan extraheras
- `HowTo`: strukturerar steg-för-steg-guider
- `Article` och `BlogPosting`: signalerar att innehållet är redaktionellt och daterat
- `Organization`: tydliggör vad bolaget gör, vilka produkter det erbjuder och var det är baserat
- `Product`: relevant för SaaS-produktsidor med tydlig beskrivning och attribut

[Google Search Central har fullständig dokumentation](https://developers.google.com/search/docs/appearance/structured-data/intro-structured-data) för alla schematyper, och implementationen kräver sällan mer än en timme per sidtyp för ett kompetent teknikteam.
...
| [Semrush AI Overviews](https://www.semrush.com/features/ai-overviews/) | Google AI Overviews-synlighet | Från $130/mån |
...
[Wikipedia är en av de mest citerade källorna i träningsdata](https://en.wikipedia.org/wiki/Wikipedia:About) för stora språkmodeller. Om ditt bolag inte har en korrekt Wikipedia-sida (eller om den är ofullständig) missar ni en direkt kanal till AI-modellernas grundkunskap. Det gäller framför allt bolag i kategorier där Wikipedia-artiklar täcker konkurrenter utförligt.
